...
const Navbar = () => {
    ...
    return (
        <div className="fixed w-full h-[8%] rounded-b-xl z-50 bg-white flex items-center dark:bg-black px-4 shadow-xl ring-1 ring-slate-100 dark:ring-0 dark:border-border dark:border-b-1">
                        ...
                        {SocialLinks.map((link) => {
                            return (
                                <Link key={link.link} href={link.link} passHref target='_blank'>
                                    <Button variant='outline' size='icon' className='bg-black text-white hover:bg-[#f5f5f0]'>
                                        {link.icon}
                                    </Button>
                                </Link>
                            )
